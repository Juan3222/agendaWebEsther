...
export default function YourComponent() {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "correos"), (snapshot) => {
      const updatedCorreos = snapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
  ...
  return (
    <>
      ...
      <div className="correos">
        {correos.map((data) => {
          return (
            <div className="lista-correos" key={data.id}>
              <p>{data.title}</p>

              <button type="button" onClick={() => handleDeleteEmail(data.id)}>
                Borrar
              </button>
            </div>
          );
